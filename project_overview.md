...
- **[2026-07-24]**: Integrated direct **YouTube Video Processing Pipeline**. Modified `dashboard/page.jsx` to process YouTube URLs natively by calling a new `/api/youtube/process` endpoint. The Next.js API triggers the `fmp-video-compressor.py` Lambda with the YouTube URL and securely passes `VIDEO_DOWNLOAD_API_KEY`. The `main.py` Fargate script now bypasses S3 downloading if a YouTube URL is present, downloads the 1080p video directly via the `p.savenow.to` external API, skips redundant FFmpeg compression, extracts audio, and uploads the final assets to S3.